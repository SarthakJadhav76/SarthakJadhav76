...
def len_search(trn_attendees,cnt):
    result = 0
...
def sent_search(trn_attendees,cnt):
    srch = int(input("Search in Database.\n\tEnter Roll Number to search :- "))
...
def bin_search(trn_attendees,srch,first,last):
...
def fib_search(trn_attendees,srch,cnt):
    n=cnt
    fibn_2=0
    fibn_1=1
    fibn=fibn_1+fibn_2
...
while(in_ch=='y' or in_ch == "Y"):
    ...
    if(opt==1):
        ...
        choose = int(input("\n\t1.Linear Search\n\t2.Sentinel Search\n\t3.Binary Search\n\t4.Fibonnaci Search :-"))
        if choose==1:
            len_search(trn_attendees,cnt)
        elif choose == 2:
            sent_search(trn_attendees,cnt)
        elif(choose==3):
            srch = int(input("Search in Database.\n\tEnter Roll Number to search :- "))
            first = 0
            last = cnt - 1
            bin_search(trn_attendees,srch,first,last)
        elif(choose == 4):
            srch = int(input("Search in Database.\n\tEnter Roll Number to search :- "))
            fib_search(trn_attendees,srch,cnt)
